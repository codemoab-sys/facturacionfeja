...
App.NewCreditNote = class NewCreditNote {
  constructor() {
    this.form = { serie: 'FC01', fecha_emision: App.todayISO(), tipo_moneda: 'PEN', doc_afectado_tipo: '01', doc_afectado_serie: 'F001', doc_afectado_correlativo: '', cod_motivo: '06', des_motivo: '' };
    this.cliente = null;
    this.items = [];
    this.sending = false;
  ...
  _bind() {
    ...
    this.container.querySelector('#f-doc-serie').addEventListener('input', function (e) { self.form.doc_afectado_serie = e.target.value.toUpperCase(); e.target.value = self.form.doc_afectado_serie; });
  ...
  async _submit() {
    ...
    try {
      ...
      new App.ResponseModal({ response: res, error: null, tipo: 'notas-credito', pdfFormat: this.pdfFormat }).render(document.body);
      this.cliente = null; this.items = [];
    } catch (e) { new App.ResponseModal({ response: null, error: e, tipo: 'notas-credito' }).render(document.body); }
    finally { btn.disabled = false; btn.innerHTML = '<i data-lucide="check" class="w-4 h-4"></i> Emitir Nota de Cr\u00e9dito'; }
  }
};
