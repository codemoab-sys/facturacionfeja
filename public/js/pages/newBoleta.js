...
(function () {
  ...
  App.NewBoleta = class NewBoleta {
    constructor() {
      this.form = { serie: 'B001', fecha_emision: App.todayISO(), tipo_moneda: 'PEN', forma_pago: 'Contado', observacion: '' };
    ...
    _renderHTML() {
      ...
      this.container.querySelector('#f-cuotas-section').style.display = f.forma_pago === 'Credito' ? 'block' : 'none';
      var clientRoot = this.container.querySelector('#f-client-selector');
      clientRoot.innerHTML = App.clientSelectorHTML(this.cliente, 'Seleccionar cliente (DNI o RUC)...');
    ...
    _bind() {
      ...
      App.bindClientSelector(this.container.querySelector('#f-client-selector'), {
        ...
        onClear: function () { self.cliente = Object.assign({}, CLIENTE_ANONIMO); self._renderHTML(); },
      });
    ...
    _cuotasRowsHTML() {
      ...
      return this.cuotas.map(function (c, i) {
        return '<div style="display: flex; gap: 0.5rem; align-items: flex-end; margin-bottom: 0.5rem;">'
          + '<div style="flex: 1;"><label class="label">Fecha pago</label><input type="date" data-ci="' + i + '" data-cf="fecha_pago" class="input" value="' + App.escapeHtml(c.fecha_pago) + '" /></div>'
          + '<div style="flex: 1;"><label class="label">Monto</label><input type="number" data-ci="' + i + '" data-cf="monto" class="input" value="' + App.escapeHtml(String(c.monto)) + '" placeholder="0.00" min="0.01" step="0.01" /></div>'
          + '<button type="button" data-cd="' + i + '" style="padding: 0.375rem; color: rgb(239 68 68); background: transparent; border: none; cursor: pointer; flex-shrink: 0; margin-bottom: 2px;"><i data-lucide="x" class="w-4 h-4"></i></button>'
        + '</div>';
      }).join('');
    }
    ...
    _bindCuotasRows() {
      ...
      rows.querySelectorAll('[data-ci]').forEach(function (el) {
        el.addEventListener('input', function () { var idx = parseInt(el.dataset.ci); var field = el.dataset.cf; self.cuotas[idx][field] = el.value; });
      });
    ...
    _calcCuotas(count) {
      ...
      for (var i = 0; i < count; i++) { var d = new Date(today); d.setMonth(d.getMonth() + i + 1); var monto = total > 0 ? (i === count - 1 ? parseFloat((total - base * (count - 1)).toFixed(2)) : base) : ''; result.push({ fecha_pago: d.toISOString().split('T')[0], monto: monto === '' ? '' : String(monto) }); }
      return result;
    }
    ...
    _openProductPicker() {
      var self = this;
      new App.ProductPicker({ onSelect: function (p) { var item = { codigo: p.codigo, cod_producto_sunat: p.cod_producto_sunat, descripcion: p.descripcion, unidad: p.unidad, cantidad: 1, precio_unitario: p.precio_unitario, tip_afe_igv: p.tip_afe_igv || '10' }; if (p.icbper) { item.icbper = p.icbper; item.factor_icbper = p.factor_icbper; } self.items.push(item); self._refreshItemsTable(); } }).render(document.body);
    }
    ...
    async _submit() {
      ...
      try {
        ...
        new App.ResponseModal({ response: res, error: null, tipo: 'boletas', pdfFormat: this.pdfFormat }).render(document.body);
        this.items = []; this.form.observacion = '';
      } catch (e) { new App.ResponseModal({ response: null, error: e, tipo: 'boletas', pdfFormat: this.pdfFormat }).render(document.body); }
      finally { this.sending = false; btn.disabled = false; btn.innerHTML = '<i data-lucide="check" class="w-4 h-4"></i> Emitir Boleta'; }
    }
  };
})();
