...
use App\Modules\Purchases\Repositories\PurchaseDetailRepository;
use App\Modules\Products\Repositories\ProductRepository;
use App\Modules\Inventory\Repositories\MovementRepository;

class PurchaseService
{
    private PurchaseRepository $repoCompra;
    private PurchaseDetailRepository $repoDetalle;
    private ProductRepository $repoProducto;
    private MovementRepository $repoMovimiento;

    public function __construct(
        ?PurchaseRepository $repoCompra = null,
        ?PurchaseDetailRepository $repoDetalle = null,
        ?ProductRepository $repoProducto = null,
        ?MovementRepository $repoMovimiento = null
    ) {
        $this->repoCompra = $repoCompra ?? new PurchaseRepository();
        $this->repoDetalle = $repoDetalle ?? new PurchaseDetailRepository();
    ...
    public function crear(int $userId, array $data): array
    {
        ...
        foreach ($detalles as $d) {
            ...
            if ($producto) {
                ...
                $precioPromedio = $nuevoStock > 0 ? ($costoTotalActual + $costoTotalNuevo) / $nuevoStock : 0;
                ...
                $this->repoMovimiento->registrar(
                    ...
                    $stockActual, $nuevoStock, 'Compra #' . $compraId, 'compra', $compraId
                );
            }
        }
